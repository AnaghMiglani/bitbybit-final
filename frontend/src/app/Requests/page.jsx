...
const BountyRequests = () => {
          ...
          try {
            ...
            if (code) {
              const response = await fetch(
                `http://localhost:4000/getAccessToken?code=${code}` // Updated port to 4000
              );
  ...
  useEffect(() => {
    const fetchBounties = async () => {
      try {
        const response = await fetch(
          "http://localhost:3000/api/contribute"
        );
        const data = await response.json();
        setUserData(data);
      } catch (error) {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      if (user) {
        try {
          const response = await fetch(
            "http://localhost:4000/getUserData", { // Updated port to 4000
              headers: {
  ...
  const handleSubmit = (e) => {
    ...
    authenticateGitHub(formData); // Ensure this function is working correctly
  };
