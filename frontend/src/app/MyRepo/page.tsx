...
const YourComponent = () => {
            ...
            async function getAccessToken() {
                try {
                    const response = await fetch(
                      "http://localhost:4000/getAccessToken?code=" + sessionStorage.getItem("code") // Updated port to 4000
                    );
    ...
    const acceptContribution = async () => {
        try {
            const response = await fetch("http://localhost:3000/api/contributeRequest", {
                method: "POST",
                headers: {
